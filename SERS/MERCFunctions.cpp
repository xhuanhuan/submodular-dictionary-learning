...
double MERCFunctions::ComputeBGain(int nVertices, int si,int sj)
{
	...
	//double b=maxCountMerged/(si+sj)*log( maxCountMerged/(si+sj))-maxCount1*log(maxCount1)-maxCount2*log(maxCount2)+1.0;
	return b;
}

double MERCFunctions::ComputeBGainSupervised(LabelVectors *labelv, int nVertices, int siID, int sjID, int si, int sj)
{
	// si, sj are the sizes of clusters being merged.
	double maxCount1 = labelv->FindRowMax(siID);
	double maxCount2 = labelv->FindRowMax(sjID);
	double maxCountMerged = labelv->FindMergedMax(siID,sjID);

	//add myself
    double sumCountMerged = labelv->FindMergedSum(siID,sjID);

	double bgain = (maxCountMerged-maxCount1-maxCount2)/(double)nVertices + 1;
    //change to
    //double bgain = maxCountMerged/(si+sj)*log( maxCountMerged/(si+sj))-maxCount1*log(maxCount1)-maxCount2*log(maxCount2)+1.0;
	// if (maxCount1>1||maxCount2>1||maxCountMerged>1)
	//mexPrintf("%f \n",bgain);
